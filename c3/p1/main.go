...
func run() error {
	...
	fmt.Println(closestIntersection(is).DistanceToOrigin())
	return nil
}

func closestIntersection(is []Intersection) Intersection {
	closest := is[0]
	for _, i := range is {
		if i.DistanceToOrigin() < closest.DistanceToOrigin() {
			closest = i
		}
	}
	return closest
...
func allIntersections(lines1, lines2 []Line) []Intersection {
...
func intersection(line1, line2 Line) (Intersection, bool) {
...
func (l Line) DistanceToOrigin() int {
